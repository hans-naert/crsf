...
/* CMSIS USART Driver for UART */
#define UART_DRIVER_NUM 6
extern ARM_DRIVER_USART ARM_Driver_USART_(UART_DRIVER_NUM);
#define ptrUART (&ARM_Driver_USART_(UART_DRIVER_NUM))
...
/* Fixed valid CRSF dummy stream transmitted on UART TX */
...
/*
  UART event callback (called from interrupt context).
  Handles receive complete and overflow events.
*/
static void uart_event_callback(uint32_t event) {
  if (event & ARM_USART_EVENT_RECEIVE_COMPLETE) {
    ...
    ptrUART->Receive((void *)rx_buffer_active, CRSF_RX_CHUNK_SIZE);
...
static int uart_init(void) {
  if (ptrUART->Initialize(uart_event_callback) != ARM_DRIVER_OK) {
    return -1;
  }

  if (ptrUART->PowerControl(ARM_POWER_FULL) != ARM_DRIVER_OK) {
    return -1;
  }

  if (ptrUART->Control(ARM_USART_MODE_ASYNCHRONOUS |
  ...
  if (ptrUART->Control(ARM_USART_CONTROL_RX, 1U) != ARM_DRIVER_OK) {
    return -1;
  }

  if (ptrUART->Control(ARM_USART_CONTROL_TX, 1U) != ARM_DRIVER_OK) {
    return -1;
  }
...
static int uart_start_receive(void) {
  ...
  if (ptrUART->Receive((void *)rx_buffer_active, CRSF_RX_CHUNK_SIZE) != ARM_DRIVER_OK) {
    return -1;
  }
  return 0;
}
...
static __NO_RETURN void thread_crsf_rx(void *argument) {
  ...
  printf("CRSF RX decoder active on UART (interrupt/DMA mode)\n");
...
/*
  UART dummy CRSF transmitter thread.
*/
static __NO_RETURN void thread_crsf_tx(void *argument) {
  ...
  printf("CRSF dummy TX active on UART\n");
  printf("Short UART TX (PA0) to UART RX (PA1) to self-test parser\n");

  osDelay(100U);

  for (;;) {
    if (ptrUART->Send(crsf_dummy_stream, (uint32_t)sizeof(crsf_dummy_stream)) != ARM_DRIVER_OK) {
      printf("Dummy TX start failed\n");
      osDelay(100U);
      continue;
    }

    while (ptrUART->GetStatus().tx_busy != 0U) {
      osDelay(1U);
    }
...
int crsf_test_start(void) {
  /* Initialize UART hardware (but don't start receive yet) */
  if (uart_init() != 0) {
    printf("UART init failed for CRSF test\n");
    return -1;
  }
  ...
  if (uart_start_receive() != 0) {
    printf("UART start receive failed\n");
    return -1;
  }
