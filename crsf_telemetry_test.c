...
static __NO_RETURN void thread_crsf_telemetry(void *argument) {
  ...
  printf("CRSF Telemetry TX active on UART\n");
  ...
  for (;;) {
    ...
    if (len > 0U) {
      if (ptrUART->Send(frame, len) == ARM_DRIVER_OK) {
...
static int UART_init_telemetry(void) {
  if (ptrUART->Initialize(NULL) != ARM_DRIVER_OK) {
    return -1;
  }
...
int crsf_telemetry_test_start(void) {
  /* Initialize UART hardware for TX only */
  if (UART_init_telemetry() != 0) {
    printf("UART init failed for telemetry test\n");
    return -1;
  }
